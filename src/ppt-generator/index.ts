 ...
 */
function parseArguments(args: string[]): PresentationConfig | null {
  ...
  try {
    ...
    for (let i = 0; i < config.slides.length; i++) {
      ...
      switch (slideConfig.type) {
        case "title":
          await addTitleSlide(pptx, slideConfig.data as TitleSlideData);
          break;
        case "content":
          await addContentSlide(pptx, slideConfig.data as ContentSlideData);
          break;
        case "image":
          await addImageSlide(pptx, slideConfig.data as ImageSlideData);
          break;
        case "closing":
 ...
 */
async function createDemoPresentation(): Promise<void> {
