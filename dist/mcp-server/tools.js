 ...
 */
export async function get_brand_guidelines() {
    try {
        const filePath = path.resolve(__dirname, '../../data/brand-guidelines.json');
        const fileContents = await fs.readFile(filePath, 'utf-8');
        const brandGuidelines = JSON.parse(fileContents);
 ...
 */
export async function search_images(query) {
    try {
 ...
 */
export async function get_image_metadata(image_id) {
    try {
        const filePath = path.resolve(__dirname, '../../data/image-repository.json');
        const fileContents = await fs.readFile(filePath, 'utf-8');
        const repository = JSON.parse(fileContents);
 ...
 */
export function search_symbols(keyword) {
 ...
 */
export async function get_partner_logo(partner_name) {
    try {
        const filePath = path.resolve(__dirname, '../../data/partner-logos.json');
        const fileContents = await fs.readFile(filePath, 'utf-8');
        const partnerData = JSON.parse(fileContents);
        const partner = partnerData.partners.find((p) => p.name.toLowerCase() === partner_name.toLowerCase());
        if (!partner) {
 ...
 */
export async function generate_presentation(slide_plan) {
    try {
 ...
 */
export async function validate_presentation(slide_plan) {
    const issues = [];
    const warnings = [];
    try {
